...
def process_pine_data(file_count, main_dir, excel_template_path, pine_id, campaign, savepath):
    """
    ...
    # -------------------------- 处理每个TXT文件 --------------------------
    # 记录成功处理的文件数
    txt_dir = main_dir + r'\raw_Data'
    if not os.path.exists(main_dir):
        print(f"警告：主目录 {main_dir} 不存在，无法处理原始数据")
        return None, None
    txt_dir = os.path.join(main_dir, 'raw_Data')
    if not os.path.exists(txt_dir):
        print(f"警告：原始数据目录 {txt_dir} 不存在，无法处理TXT文件")
        return None, None
    success_count = 0
