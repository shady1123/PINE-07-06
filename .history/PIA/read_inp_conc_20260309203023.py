...
def read_input_conc(OP_ID, MAIN_DIR, PINE_ID, CAMPAIGN):
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if 'run_id' in row_items and 'datetime' in row_items:
            ...
            print(f"\n✅ 成功识别表头行(行号{line_num})，表头列: {header}")
            print(f"   预计数据从行号 {data_start_line_num} 开始\n")
            # 建立列名-索引映射
        ...
        # ============== 核心调试区域：详细打印每一行的处理情况 ==============
        print(f"正在处理行号 {line_num} (距表头第 {line_num - data_start_line_num + 1} 条数据):")
        print(f"  原始内容(前100字符): {strip_line[:100]}")
        ...
        print(f"  分割后列数: {len(row_items)}, 内容: {row_items}")
    ...
    if not data:
        raise ValueError("文件中未读取到有效数据，请根据上方调试打印检查具体原因")
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f'{SAVEPATH}/INP_concentration_over_time.png')
