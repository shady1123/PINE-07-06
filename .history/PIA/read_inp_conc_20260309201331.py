...
def read_input_conc(OP_ID,MAIN_DIR, PINE_ID, CAMPAIGN):
    ...
    for line in lines:
        if line.startswith('Run ID'):
            ...
            chamber = line.split(':')[1].strip()
        elif line.startswith('T_min'):
            T_min = line.split(':')[1].strip()
        elif line.startswith('P_end'):
            P_end = line.split(':')[1].strip()
        elif line.startswith('Fe_mean'):
            Fe_mean = line.split(':')[1].strip()
        elif line.startswith('INP_count'):
            INP_count = line.split(':')[1].strip()
        elif line.startswith('INP_cn_0'):
            INP_cn_0 = line.split(':')[1].strip()
        elif line.startswith('INP_cn_flush'):
            INP_cn_flush = line.split(':')[1].strip()
        elif line.startswith('spd_cn_0'):
            spd_cn_0 = line.split(':')[1].strip()
        elif line.startswith('spd_cn_flush'):
            spd_cn_flush = line.split(':')[1].strip()
    # 将提取的数据添加到data列表中
    data.append([run_id, date, time, chamber, T_min, P_end, Fe_mean, INP_count, INP_cn_0, INP_cn_flush, spd_cn_0, spd_cn_flush])

    # 画图
    import matplotlib.pyplot as plt
   
    # data列表datatime、INP_cn_0画图
    datatime = [f"{date} {time}"]
