...
def read_input_conc(OP_ID, MAIN_DIR, PINE_ID, CAMPAIGN):
    ...
    # 画散点图，展示INP_cn_0随时间的变化趋势
    plt.figure(figsize=(12, 6))
    plt.scatter(datetime_objs, inp_cn_0_list, color='#1f77b4', alpha=0.4, s=20)
    plt.xlabel('Datetime')
    ...
    plt.tight_layout()
    # 横轴不留白，直接从数据范围开始
    plt.xlim(datetime_objs[0], datetime_objs[-1])
    print(datetime_objs[0], datetime_objs[-1])
    # 纵轴对数坐标
    plt.yscale('log')
    # plt.show()
    plt.savefig(f'{SAVEPATH}/{PINE_ID}_{CAMPAIGN}_op_id_{OP_ID}_INP_concentration.png')
    print(f"图表已保存到: {SAVEPATH}/{PINE_ID}_{CAMPAIGN}_op_id_{OP_ID}_INP_concentration.png")
    plt.close()
